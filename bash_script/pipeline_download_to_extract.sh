...
mkdir -p ${Dir}ariba_run_report
mkdir -p ${Dir}ariba_run_output
...
do
        if [ -f "${OutputDir}${Folder}/report.tsv" ]; then
                ...
                ariba run /home/fortissible/skripsi/ariba_prefref/prepareref.out.card ${Dir}${Folder}/${Folder}_1.fastq ${Dir}${Folder}/${Folder}_2.fastq ${Dir}ariba_run_output/${Folder}/ --force --verbose --noclean --threads 8
